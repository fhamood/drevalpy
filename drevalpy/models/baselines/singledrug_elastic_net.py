...
class SingleDrugPhosphoProteomicsElasticNet(SingleDrugElasticNet):
    """SingleDrugProteomicsElasticNet class."""
    ...
    def load_cell_line_features(self, data_path: str, dataset_name: str) -> FeatureDataset:
        """
        Loads the phosphoproteomics data.

        :param data_path: path to the data
        :param dataset_name: name of the dataset
        :returns: phosphoproteomics data
        """
        return load_and_select_gene_features(
            feature_type="phosphoproteomics",
            gene_list=None,
            data_path=data_path,
            dataset_name=dataset_name,
        )
    ...
    def train(
        ...
        if len(output) > 0:
            ...
            x = self.get_concatenated_features(
                cell_line_view="phosphoproteomics",
                drug_view=None,
                cell_line_ids_output=output.cell_line_ids,
                drug_ids_output=output.drug_ids,
                cell_line_input=cell_line_input,
